        ...
        with open(self.filename) as f:
            ...
            for line in lines:
                if line.find("DATE") == -1: ## EXCLUDE HEADER
                    ...
                    crime_date = line_content[len(line_content)-4]
                    crime_key = crime_type + "," + crime_id
                    self.crime_counter[crime_key] = self.crime_counter.get(crime_key + "", 0) + 1
                    old_date = self.crime_dates[crime_key]
                    if old_date < crime_date: ## updating recent crime date
                        self.crime_dates[crime_key] = crime_date
